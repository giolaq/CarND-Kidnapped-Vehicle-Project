...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    // add noise
    p.x += N_x_init(random_generator);
    p.y += N_y_init(random_generator);
    p.theta += N_theta_init(random_generator);
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
 ...
 for (int i = 0; i < num_particles; i++) {
   ...
   // add noise
   particles[i].x += N_x(random_generator);
   particles[i].y += N_y(random_generator);
   particles[i].theta += N_theta(random_generator);
   }
}
...
void ParticleFilter::resample() {
   ...
   auto index = uniintdist(random_generator);
